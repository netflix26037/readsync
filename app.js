...
// This is the "password" for your personal reader. Set it as an
// environment variable (locally, or in your Vercel project settings)
...
app.delete('/api/feeds/:feedId', auth, async (req, res) => {
  const db = await loadDB();
...
// "Read" is a single list stored in Redis, so opening this same app from
// any device always shows the same read/unread state, permanently.
// ---------------------------------------------------------------------------
...
app.post('/api/articles/:articleId/read', auth, async (req, res) => {
  const db = await loadDB();
...
app.post('/api/articles/:articleId/unread', auth, async (req, res) => {
  const db = await loadDB();
